// Avant - Game System for FoundryVTT v13
// Generated by MCP AI Assistant

console.log("Avant | Loading game system...");

/**
 * Actor Data Model for Avant
 */
class AvantActorData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
    ...
    prepareBaseData() {
        ...
        this.abilities.might.mod = Math.floor((this.abilities.might.value - 10) / 2);
        this.abilities.grace.mod = Math.floor((this.abilities.grace.value - 10) / 2);
        this.abilities.intellect.mod = Math.floor((this.abilities.intellect.value - 10) / 2);
        this.abilities.focus.mod = Math.floor((this.abilities.focus.value - 10) / 2);
        
        // Calculate derived stats
        this.defenseThreshold = 11 + this.level + this.abilities.grace.mod;
        
        // Ensure health is within bounds
        if (this.health.value > this.health.max + this.health.temp) {
            this.health.value = this.health.max + this.health.temp;
        }
        
        // Calculate remaining expertise points
        this.expertisePoints.remaining = this.expertisePoints.total - this.expertisePoints.spent;
        
        // Ensure power points are within bounds
    ...
    prepareDerivedData() {
        ...
        // Calculate weapon thresholds for all weapons
        if (this.parent && this.parent.items) {
            for (let item of this.parent.items) {
                if (item.type === 'weapon' && item.system) {
                    const weaponAbility = item.system.ability || 'might';
                    const abilityMod = this.abilities[weaponAbility]?.mod || 0;
                    const weaponModifier = item.system.modifier || 0;
                    item.system.threshold = 11 + this.level + abilityMod + weaponModifier;
                }
                if (item.type === 'armor' && item.system) {
                    const armorAbility = item.system.ability || 'grace';
                    const abilityMod = this.abilities[armorAbility]?.mod || 0;
                    const armorModifier = item.system.modifier || 0;
                    item.system.threshold = 11 + this.level + abilityMod + armorModifier;
                }
            }
        }
    }
 ...
 * Item Data Models
 */
class AvantActionData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
        const fields = foundry.data.fields;
        return {
...
class AvantFeatureData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
        const fields = foundry.data.fields;
        return {
            description: new fields.HTMLField({
            ...
            category: new fields.StringField({
                required: true,
                initial: "general",
                choices: ["ancestry", "culture", "lineage", "vocation", "general"]
            }),
...
class AvantTalentData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
        const fields = foundry.data.fields;
        return {
            description: new fields.HTMLField({
...
class AvantAugmentData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
        const fields = foundry.data.fields;
        return {
            description: new fields.HTMLField({
            ...
            augmentType: new fields.StringField({
                required: true,
                initial: "enhancement",
                choices: ["enhancement", "modification", "implant", "symbiotic"]
            }),
            prerequisites: new fields.StringField({
                required: false,
                blank: true,
...
class AvantWeaponData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
        const fields = foundry.data.fields;
        return {
            description: new fields.HTMLField({
    ...
    prepareBaseData() {
        ...
        // Calculate threshold if we have access to actor data
        // This will be properly calculated in the actor's prepareData method
        if (!this.threshold || this.threshold === 11) {
            this.threshold = 11; // Default base threshold
        }
    }
}

class AvantArmorData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
        const fields = foundry.data.fields;
        return {
            description: new fields.HTMLField({
...
class AvantGearData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
        const fields = foundry.data.fields;
        return {
            description: new fields.HTMLField({
 ...
 * Actor Sheet
 */
class AvantActorSheet extends foundry.appv1.sheets.ActorSheet {
 ...
 * Item Sheet
 */
class AvantItemSheet extends foundry.appv1.sheets.ItemSheet {
    static get defaultOptions() {
        return foundry.utils.mergeObject(super.defaultOptions, {
...
Hooks.once('init', async function() {
    ...
    foundry.documents.collections.Actors.registerSheet("avant", AvantActorSheet, { 
        makeDefault: true,
        types: ["character", "npc", "vehicle"]  // Specify supported types
    });
    
    foundry.documents.collections.Items.unregisterSheet("core", foundry.appv1.sheets.ItemSheet);
    foundry.documents.collections.Items.registerSheet("avant", AvantItemSheet, { 
        makeDefault: true,
